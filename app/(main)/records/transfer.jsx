...
import TransferRecord from "../../../components/reuseables/TransferRecord";
...
export default function TransferRecords() {
  ...
  const search_transfers = async (page) => {
    const { q, sort } = formData;
    const res = await TRANSFER_HOOKS.fetch_multiple_transfers(
      setIsLoading,
      page,
      sort,
      q
  ...
  const see_more = DEBOUNCE(async () => {
    if (Boolean(meta?.has_next_page)) {
      search_transfers(Number(meta?.page + 1));
    }
  });
  ...
  return (
    <SafeAreaWrapper>
      ...
      <PageSearchComponent
        form={formData}
        setForm={setFormData}
        placeholder={"Search for cashouts"}
        ...
        <SectionGroupWrapper>
          {transfers?.length > 0 ? (
            transfers?.map((item, index) => (
              <TransferRecord key={index} data={item} />
            ))
          ) : (
            <NotFoundComponent
              text={"No cashout found"}
              isLoading={isLoading}
            />
          )}
